"""
Fast Leaguepedia Fetcher — uses ScoreboardPlayers for speed
CS 163 Group 10 - Brandon Lee & Stephani Marie Soriano

Strategy: Query ScoreboardPlayers joined with ScoreboardGames per season.
This table has one row per player per game (not per tournament), so we get
full coverage in ~3 pages per season instead of 20+.

Then deduplicate to one row per player per season, picking their highest-tier team.
...
PAGE_SIZE    = 5000  # max allowed — fewer pages = faster
PAGE_DELAY   = 3     # short delay between pages
SEASON_DELAY = 5     # short delay between seasons
RETRY_WAIT   = 60
MAX_RETRIES  = 4
PROGRESS_FILE = "fetch_progress.json"

LEAGUE_KEYWORDS = {
    'LoL Champions Korea': 'LCK', 'Challengers Korea': 'LCK', 'KeSPA': 'LCK',
    'Tencent LoL Pro League': 'LPL', 'LPL': 'LPL',
    'LoL EMEA Championship': 'LEC', 'Europe League Championship Series': 'LEC', 'LEC': 'LEC',
    'League Championship Series': 'LCS', 'NA LCS': 'LCS', 'North American': 'LCS', 'LCS': 'LCS',
    'Pacific Championship Series': 'PCS', 'LoL Master Series': 'LMS', 'LMS': 'LMS',
    'Turkish Championship League': 'TCL', 'TCL': 'TCL',
    'Vietnam Championship Series': 'VCS', 'VCS': 'VCS',
    'CBLOL': 'CBLOL', 'Latin America': 'LLA', 'LLA': 'LLA',
    'Oceanic': 'OCE', 'OPL': 'OCE',
}
REGION_MAP = {
...
def fetch_season(site, date_start, date_end):
    """
    Pull from ScoreboardPlayers + ScoreboardGames — one row per player per game.
    Much fewer total rows than TournamentPlayers, fetches faster.
    """
    all_rows = []
    offset = 0
    page = 1
    while True:
        ...
        rows = query_with_retry(
            ...
            fields=(
                "ScoreboardPlayers.Name=Player,"
                "ScoreboardPlayers.Team=Team,"
            ...
            join_on=(
                ...
                "ScoreboardGames.Tournament=Tournaments.OverviewPage"
            ),
...
def process_rows(rows, csv_players, season):
    year = str(2015 + season)
    player_best = {}
    for row in rows:
        ...
        if not player or not team or player not in csv_players:
            continue
        if not str(row.get("DateStart") or "").startswith(year):
            continue
...
if os.path.exists(PROGRESS_FILE):
    ...
    print(f"  Completed seasons: {sorted(completed_seasons)}")
else:
    print(f"\nStarting fresh...")
...
for season, (date_start, date_end) in SEASONS.items():
    ...
    if season in completed_seasons:
        print(f"[Season {season}] Already done — skipping")
        continue

    print(f"[Season {season}] {date_start} → {date_end}")
    rows = fetch_season(site, date_start, date_end)
    ...
    player_best = process_rows(rows, csv_players, season)
...
for season, df in season_dfs.items():
    ...
    total  = len(merged)
    print(f"  S{season}: {filled}/{total} ({filled/total*100:.1f}%) players matched")
